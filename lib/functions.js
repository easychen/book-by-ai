...
export async function genImage( text, style = '' )
{
    ...
    const url = process.env.STABILITY_API_ENDPOINT + '/v1/generation/stable-diffusion-xl-1024-v1-0/text-to-image';
...
export function extractJSON(dialogue,tag = null) {
    ...
    if( tag )
    {
        ...
        // 如果有多个匹配，取最后一个
        const the_match = match && match[match.length-1] ? match[match.length-1]: null;
        return JSON.stringify({[tag]:the_match});
    }else
    {
        const reg = /^.+?(\{.+\}).*?$/gs;
        const match = reg.exec(` ${dialogue} `);
...
export async function search( query, sites = false, headless = true, extend = false )
{
    ...
    for (let result of results) {
        let markdown = await fetch_url2(result.link, 5000);
        if (markdown) {
...
export async function fetch_url2(url, length = 3000, timeout = 10*1000) {
    let opt = {
        args: ['--no-sandbox'],
        defaultViewport: null,
        headless:true,
...
export async function fetch_url(url, length = 3000, timeout = 10*1000) {
  try {
    ...
    const charset = getCharset(buffer, response.headers);
...
function getCharset(buffer, headers) 
{
    // 首先从 headers - content-type 中获取 charset
    ...
    console.log("getCharset", str);
    // 然后匹配 <meta charset="utf-8" />
    const match = str.match(/<meta.+?charset="(.+?)".*?>/);
    if (match) {
        return match[1];
    }
    // 如果还没有，那么就返回 utf8
...
export function convertToDirectoryPath(bookName) {
    // 去掉引号
    let path = bookName.replace(/'/g, '');
    // 将空格和冒号替换为下划线
