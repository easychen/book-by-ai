...
export async function search( query, sites = false, headless = true, extend = false )
{
    ...
    const browser = await puppeteer.launch(opt);
    const page = await browser.newPage();
    const url = 'https://www.google.com/search?q=' + encodeURIComponent(sites ? sites.map( site => `site: ${site}` ).join(' ') + ' ' + query : query);
    ...
    for (let result of results) {
        let markdown = await fetch_url(result.link);
        if (markdown) {
...
export async function fetch_url(url, length = 3000) {
  try {
